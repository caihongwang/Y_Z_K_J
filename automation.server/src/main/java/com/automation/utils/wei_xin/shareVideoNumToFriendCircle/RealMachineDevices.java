...
//                        "[\"内部交流群\"]";
        ...
        try {
            ...
            e.printStackTrace();
            throw new Exception("【分享视频号到朋友圈】配置连接android驱动出现异常,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】Appium端口号【" + appiumPort + "】的环境是否正常运行等原因....");
        }
        for (int i = 1; i <= 30; i++) {     //每间隔5秒点击一次，持续90秒
            ...
            try {
                ...
                logger.info("【分享视频号到朋友圈】点击坐标【搜索框】成功....");
                Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
            } catch (Exception e) {
                logger.info("【分享视频号到朋友圈】点击坐标【搜索框】失败，因为微信正在建立索引....");
                if (i == 30) {
                    throw new Exception("【分享视频号到朋友圈】点击坐标【搜索框】均失败,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
                } else {
                    Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
                    logger.info("【分享视频号到朋友圈】第【" + i + "】次点击坐标【搜索框】失败，因为微信正在建立索引....");
                    continue;
                }
            }
            //3.点击坐标【搜索输入框】
            try {
                ...
                logger.info("【分享视频号到朋友圈】点击坐标【输入昵称到搜索框:text/搜索】成功....");
                Thread.sleep(1000);
                break;
            } catch (Exception e) {
                logger.info("【分享视频号到朋友圈】点击坐标【输入昵称到搜索框:text/搜索】失败....");
                try {
                    driver.findElementByAndroidUIAutomator("new UiSelector().className(\"android.widget.EditText\")").sendKeys(targetGroup);
                    logger.info("【分享视频号到朋友圈】点击坐标【输入昵称到搜索框:className/android.widget.EditText】成功....");
                    Thread.sleep(1000);
                    break;
                } catch (Exception e1) {
                    logger.info("【分享视频号到朋友圈】点击坐标【输入昵称到搜索框:className/android.widget.EditText】失败....");
                    if (i == 30) {
                        throw new Exception("【分享视频号到朋友圈】点击坐标【输入昵称到搜索框:text/搜索】与【输入昵称到搜索框:className/android.widget.EditText】均失败,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
                    } else {
                        Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
                        logger.info("【分享视频号到朋友圈】第【" + i + "】次点击坐标【输入昵称到搜索框:text/搜索】与【输入昵称到搜索框:className/android.widget.EditText】均失败，因为微信正在建立索引....");
                        continue;
                    }
                }
            }
        ...
        try {
            ...
            try {
                ...
                throw new Exception("【分享视频号到朋友圈】判断坐标【联系人】与【最常使用】均不存在，当前昵称【" + targetGroup + "】对应的可能是【微信群】或者【公众号】或者【聊天记录】....");
        ...
        try {
            ...
            logger.info("【分享视频号到朋友圈】点击坐标【昵称对应的微信好友群】通过【联系人的xpath】成功....");
            Thread.sleep(1000);
        } catch (Exception e) {
            try {
                driver.findElementByXPath("//android.widget.TextView[@text=\"" + mostUsedLocaltion + "\"]/../../../android.widget.RelativeLayout[2]").click();
                logger.info("【分享视频号到朋友圈】点击坐标【昵称对应的微信好友群】通过【最常使用的xpath】成功....");
                Thread.sleep(1000);
            } catch (Exception e1) {
                throw new Exception("【分享视频号到朋友圈】通过【联系人的xpath】与【最常使用的xpath】点击坐标【昵称对应的微信好友】均失败，当前昵称【\" + nickName + \"】对应的可能是【微信群】或者【公众号】或者【聊天记录】....");
            }
        }
        ...
        while (iterator.hasNext()) {
            ...
            try {
                ...
                logger.info("【分享视频号到朋友圈】获取所有的微信消息RelativeLayout 成功....");
                Thread.sleep(1000);
            } catch (Exception e) {
                throw new Exception("【分享视频号到朋友圈】获取所有的微信消息RelativeLayout 异常....");
            }
            ...
            try {
                ...
                logger.info("【分享视频号到朋友圈】长按最后一条消息等待弹出【多选等弹窗】成功....");
                Thread.sleep(1000);
            } catch (Exception e) {
                e.printStackTrace();
                throw new Exception("【分享视频号到朋友圈】长按最后一条消息等待弹出【多选等弹窗】异常，" + e.getMessage());
            }
            ...
            try {
                ...
                try {
                    ...
                    throw new Exception("【分享视频号到朋友圈】长按最后一条消息等待弹出【多选等弹窗】异常....");
                }
            }
            ...
            while (true) {
                try {
                    ...
                    logger.info("【分享视频号到朋友圈】点击坐标【获取所有单选框CheckBox】成功....");
                    for (int i = (checkBoxWebElementList.size() - 1); i >= 0; i--) {
                        try {
                            ...
                            logger.info("【分享视频号到朋友圈】点击坐标【单条微信消息CheckBox】异常....");
                        } finally {
                    ...
                    logger.info("【分享视频号到朋友圈】第【" + getAllCheckBoxWebNum + "】次获取所有【微信消息CheckBox】异常....");
                    if (getAllCheckBoxWebNum >= 10) {
                        logger.info("【分享视频号到朋友圈】第【" + getAllCheckBoxWebNum + "】次获取所有【微信消息CheckBox】异常，" + e.getMessage());
                    }
                } finally {
                    if (relayTheWxMessageNum_selected >= relayTheWxMessageNum) {
                        break;
                        ...
                        try {
                            ...
                            logger.info("【分享视频号到朋友圈】设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】【下滑】显示更多需要转发的微信消息....");
                            Thread.sleep(1000);
                        } catch (Exception e) {
                            logger.info("【分享视频号到朋友圈】设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】【下滑】显示更多需要转发的微信消息....");
                        }
            ...
            try {
                ...
                logger.info("【分享视频号到朋友圈】点击坐标【分享】成功....");
                Thread.sleep(1000);
            } catch (Exception e) {
                throw new Exception("【分享视频号到朋友圈】点击坐标【分享】异常....");
            }
            //点击坐标【逐条转发】
            try {
                driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + forwardOneByOneLocaltion + "\")").click();
                logger.info("【分享视频号到朋友圈】点击坐标【逐条转发】成功....");
                Thread.sleep(1000);
            } catch (Exception e) {
                throw new Exception("【分享视频号到朋友圈】点击坐标【逐条转发】异常....");
            }
            //点击坐标【多选】
            try {
                driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + multipleChoiceLocaltion + "\")").click();
                logger.info("【分享视频号到朋友圈】点击坐标【多选】成功....");
                Thread.sleep(1000);
            } catch (Exception e) {
                throw new Exception("【分享视频号到朋友圈】点击坐标【多选】异常....");
            }
            while (true) {
                //点击坐标【搜索】
                try {
                    ...
                    logger.info("【分享视频号到朋友圈】点击坐标【搜索[android.widget.EditText]并输入群名:" + relayTargetGroup + "】成功....");
                    Thread.sleep(1000);
                } catch (Exception e) {
                    throw new Exception("【分享视频号到朋友圈】点击坐标【搜索[android.widget.EditText]并输入群名:" + relayTargetGroup + "】异常....");
                }
                //点击坐标【群名】           //android.widget.TextView[@text="内部交流群"]
                try {
                    driver.findElementByXPath("//android.widget.TextView[@text=\"" + relayTargetGroup + "\"]").click();
                    logger.info("【分享视频号到朋友圈】点击坐标【群名：" + relayTargetGroup + "】【xpath】成功....");
                    Thread.sleep(1000);
                } catch (Exception e) {
                    logger.info("【分享视频号到朋友圈】点击坐标【群名：" + relayTargetGroup + "】【xpath】异常....");
                }
                relayNumOfOne++;
                if (relayNumOfOne >= 9 || relayNumOfOne >= relayTargetGroupList.size() || !iterator.hasNext()) {
                    ...
                    try {
                        ...
                        logger.info("【分享视频号到朋友圈】点击坐标【发送(】成功....");
                        Thread.sleep(1000);
                    } catch (Exception e) {
                        throw new Exception("【分享视频号到朋友圈】点击坐标【发送(】异常....");
                    }
                    //点击坐标【发送】
                    try {
