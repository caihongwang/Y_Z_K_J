...
import com.newMall.utils.ALiYunHttpUtils;
import com.newMall.utils.HttpsUtil;
import com.newMall.utils.IOUtils;
import com.newMall.utils.MediaArticlesUtil;
import com.newMall.utils.MediaTypeUtil;
import com.newMall.utils.XmlUtil;
import com.newMall.utils.wxpay.WXPay;
import com.newMall.utils.wxpay.WXPayConfigImpl;
...
public class WX_PublicNumberUtil {
    ...
    private static final Logger logger = LoggerFactory.getLogger(com.newMall.utils.WX_PublicNumberUtil.class);
        ...
        if (!"".equals(data) && !"".equals(form_id)
            ...
            try {
                ...
                if (resultMap.get("access_token") != null
                    ...
                    HttpResponse response = com.newMall.utils.ALiYunHttpUtils.doPost(host, "", method, headers, querys, bodys);
    ...
    public static Map<String, Object> getMiniOpenIdAndSessionKeyForWX(Map<String, Object> paramMap) {
        com.newMall.utils.HttpsUtil httpsUtil = new com.newMall.utils.HttpsUtil();
     ...
     * @return ApiResult
     */
    public static Map<String, Object> uploadMedia(com.newMall.utils.MediaTypeUtil mediaTypeUtil, File file) {
        com.newMall.utils.HttpsUtil httpsUtil = new com.newMall.utils.HttpsUtil();
        Map<String, String> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
        Map<String, Object> accessTokenMap = getAccessToken();
    ...
    public static Map<String, Object> getMedia(String media_id) {
        ...
        if (accessTokenMap != null && accessTokenMap.size() > 0) {
            ...
            try{
                com.newMall.utils.MediaFileUtil mediaFileUtil = httpsUtil.download(get_media_url);
     ...
     * @return ApiResult
     */
    public static Map<String, Object> addMaterial(File file, com.newMall.utils.MediaTypeUtil mediaTypeUtil) {
        com.newMall.utils.HttpsUtil httpsUtil = new com.newMall.utils.HttpsUtil();
     ...
     * @return ApiResult
     */
    public static Map<String, Object> addNews(List<com.newMall.utils.MediaArticlesUtil> mediaArticlesUtilList) {
        com.newMall.utils.HttpsUtil httpsUtil = new com.newMall.utils.HttpsUtil();
        Map<String, Object> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
        Map<String, Object> accessTokenMap = getAccessToken();
    ...
    public static List<Map<String, Object>> batchGetMaterial(MediaTypeUtil mediaTypeUtil, Integer offset, Integer count) {
        com.newMall.utils.HttpsUtil httpsUtil = new HttpsUtil();
        Map<String, Object> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
...
//                        offset = offset + 2;
    ...
    public static ResultMapDTO enterprisePayment(
        ...
        String xml = com.newMall.utils.XmlUtil.toXml(params);
        System.out.println(xml);
        String xmlResult = postSSL(transfer_uri, xml, certPath, mchId);
        System.out.println(xmlResult);
        try {
            Map<String, String> resultXML = com.newMall.utils.XmlUtil.xml2map(xmlResult.toString(), false);
    ...
    public static boolean sendRedPack(
        ...
        try {
            Map<String, String> result = com.newMall.utils.XmlUtil.xml2map(xmlResult, false);
    ...
    public static String sendRedPack(Map<String, String> params, String certPath, String partner) {
        return postSSL(sendRedPack_uri, com.newMall.utils.XmlUtil.toXml(params), certPath, partner);
    }
