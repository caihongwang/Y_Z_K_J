...
public class WXPayReport {
    ...
    public static class ReportInfo {
        ...
        public String toLineString(String key) {
            ...
            try {
                String sign = com.newMall.utils.wxpay.WXPayUtil.HMACSHA256(sb.toString(), key);
                sb.append(sign);
                return sb.toString();
            } catch (Exception ex) {
                return null;
    ...
    private volatile static com.newMall.utils.wxpay.WXPayReport INSTANCE;

    private WXPayReport(final com.newMall.utils.wxpay.WXPayConfig config) {
        this.config = config;
        ...
        if (config.shouldAutoReport()) {
            com.newMall.utils.wxpay.WXPayUtil.getLogger().info("report worker num: {}", config.getReportWorkerNum());
                    ...
                    public void run() {
                        while (true) {
                            ...
                            try {
                                ...
                                com.newMall.utils.wxpay.WXPayUtil.getLogger().info("get first report msg: {}", firstMsg);
                                String msg = null;
                                sb.append(firstMsg); //会阻塞至有消息
                                int remainNum = config.getReportBatchSize() - 1;
                                for (int j = 0; j < remainNum; ++j) {
                                    com.newMall.utils.wxpay.WXPayUtil.getLogger().info("try get remain report msg");
                                    // msg = reportMsgQueue.poll();  // 不阻塞了
                                    msg = reportMsgQueue.take();
                                    com.newMall.utils.wxpay.WXPayUtil.getLogger().info("get remain report msg: {}", msg);
                                    if (msg == null) {
                                        break;
                                    } else {
                                        sb.append("\n");
                                ...
                                // 上报
                                com.newMall.utils.wxpay.WXPayReport.httpRequest(sb.toString(), DEFAULT_CONNECT_TIMEOUT_MS, DEFAULT_READ_TIMEOUT_MS);
                            } catch (Exception ex) {
                                com.newMall.utils.wxpay.WXPayUtil.getLogger().warn("report fail. reason: {}", ex.getMessage());
                            }
                        }
                    }
                });
     ...
     * @return
     */
    public static com.newMall.utils.wxpay.WXPayReport getInstance(WXPayConfig config) {
        if (INSTANCE == null) {
            synchronized (com.newMall.utils.wxpay.WXPayReport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new com.newMall.utils.wxpay.WXPayReport(config);
                }
            }
        }
        return INSTANCE;
    ...
    public void report(String uuid, long elapsedTimeMillis,
        ...
        long currentTimestamp = com.newMall.utils.wxpay.WXPayUtil.getCurrentTimestamp();
        ...
        com.newMall.utils.wxpay.WXPayUtil.getLogger().info("report {}", data);
        if (data != null) {
