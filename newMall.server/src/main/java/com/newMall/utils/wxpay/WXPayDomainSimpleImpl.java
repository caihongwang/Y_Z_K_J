 ...
 */
public class WXPayDomainSimpleImpl implements com.newMall.utils.wxpay.IWXPayDomain {
    private WXPayDomainSimpleImpl() {
    }

    private static class WxpayDomainHolder {
        private static com.newMall.utils.wxpay.IWXPayDomain holder = new com.newMall.utils.wxpay.WXPayDomainSimpleImpl();
    }
    ...
    public synchronized DomainInfo getDomain(final WXPayConfig config) {
        DomainStatics primaryDomain = domainData.get(com.newMall.utils.wxpay.WXPayConstants.DOMAIN_API);
        if (primaryDomain == null ||
                primaryDomain.isGood()) {
            return new DomainInfo(com.newMall.utils.wxpay.WXPayConstants.DOMAIN_API, true);
        }

        long now = System.currentTimeMillis();
        if (switchToAlternateDomainTime == 0) {   //first switch
            switchToAlternateDomainTime = now;
            return new DomainInfo(com.newMall.utils.wxpay.WXPayConstants.DOMAIN_API2, false);
        } else if (now - switchToAlternateDomainTime < MIN_SWITCH_PRIMARY_MSEC) {
            DomainStatics alternateDomain = domainData.get(com.newMall.utils.wxpay.WXPayConstants.DOMAIN_API2);
