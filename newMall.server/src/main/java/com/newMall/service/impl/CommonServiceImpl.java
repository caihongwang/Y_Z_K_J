...
import com.newMall.utils.jsapi.SignUtil;
import com.newMall.dao.CustomMessageHistoryDao;
import com.newMall.dao.WX_DicDao;
...
public class CommonServiceImpl implements CommonService {
    ...
    @Autowired
    private com.newMall.utils.HttpsUtil httpsUtil;
...
//        resultMap.putAll(SignUtil.getSign("http://127.0.0.1:8080//newMall/wx_sdk.html?uid=1"));
    ...
    public ResultMapDTO sendTemplateMessageForMiniProgram(Map<String, Object> paramMap) {
        ...
        if (!"".equals(data) && !"".equals(form_id)
            ...
            try {
                Map<String, Object> resultMap = com.newMall.utils.WX_PublicNumberUtil.getAccessToken(NewMallCode.WX_MINI_PROGRAM_APPID, NewMallCode.WX_MINI_PROGRAM_SECRET);
    ...
    public ResultMapDTO sendTemplateMessageForWxPublicNumber(Map<String, Object> paramMap) {
        ...
        if (!"".equals(data) && !"".equals(template_id)&& !"".equals(openId)) {
            try {
                Map<String, Object> resultMap = com.newMall.utils.WX_PublicNumberUtil.getAccessToken(NewMallCode.WX_PUBLIC_NUMBER_APPID, NewMallCode.WX_PUBLIC_NUMBER_SECRET);
                if (resultMap.get("access_token") != null
                        && !"".equals(resultMap.get("access_token").toString())) {
                    String accessToken = resultMap.get("access_token").toString();
                    paramMap.put("touser", openId);
                ...
                if ("".equals(Content) && "gh_bcce99ab0079".equals(ToUserName)) {
                    ...
                    if(customMessageResultDTO != null && customMessageResultDTO.getResultList() != null
                        ...
                        if(!"".equals(title) && !"".equals(appid)
                            ...
                            customMessageStr = com.newMall.utils.CustomMessageUtil.makeMiniProgramPageCustomMessage(
                                    FromUserName,
                                    title,
                                    appid,
                                    pagePath,
                    ...
                    resultMap = com.newMall.utils.WX_PublicNumberUtil.getAccessToken(customMessageAccountAppId, customMessageAccountSecret);
                    if (resultMap.get("access_token") != null &&
                            !"".equals(resultMap.get("access_token").toString())) {
                        String accessToken = resultMap.get("access_token").toString();
                        ...
                        try {
                            HttpResponse response = com.newMall.utils.ALiYunHttpUtils.doPost(host, path, method, headers, querys, bodys);
                            ...
                            customMessageHistoryMap.put("createTime", com.newMall.utils.TimestampUtil.getTimestamp());
                            customMessageHistoryMap.put("updateTime", com.newMall.utils.TimestampUtil.getTimestamp());
                ...
                if (!"".equals(Content) && pattern.matcher(Content).matches()) {
                    ...
                    if (!"".equals(customMessageType) && !"".equals(customMessageAccountName)
                        ...
                        if(dicResultDTO != null && dicResultDTO.getResultList() != null
                            ...
                            //图片消息
                            customMessageStr = com.newMall.utils.CustomMessageUtil.makeImageCustomMessage(FromUserName, title, description, imgUrl);
                        }
                    }
                    //准备 客服消息历史记录 参数
                    customMessage_customMessageType = customMessageType;
