...
//        do_OilPrizeMessage_For_OilStationMap();

//        do_LoginAndOperator_For_TrsafetyPlatform();

        do_getOilPriceFromOilUsdCnyCom_For_OilStationMap();
    ...
    /**
     * 每天下午16:00，定时提交铜仁市安全生产双控信息系统的隐患检查登记
     * 铜仁市安全生产双控信息系统的隐患检查登记
     */
    public void do_LoginAndOperator_For_TrsafetyPlatform()     {
        logger.info("【定时任务】铜仁市安全生产双控信息系统的隐患检查登记 当前环境 useEnvironmental = " + useEnvironmental);
        if (useEnvironmental != null && "prepub".equals(useEnvironmental)) {
            try {
                //1.准备参数
                String username = "caizhiwen";          //用户名
                String password = "caizhiwen";          //密码
                String loginUrl = "http://www.toosafety.com/qiyesafety/html/user/login/main?servername=trsafety";          //登录url
                String operatorUrl = "http://www.toosafety.com/qiyesafety/json/danger/add3";   //操作url
                //准备 隐患排查 项目名称
                List<String> operatorList = Lists.newArrayList();
                //第一横排
                operatorList.add("安全目标-风险检查");
...
//                operatorList.add("作业许可");
                ...
                logger.info("用户名密码登陆--->>状态:" + ("302".equals(loginResponseState.getStatusCode()+"") ? "登录成功" : "登录失败"));
                ...
                for (String operatorNameStr : operatorList) {
                    ...
                    for (int i = 0; i < headers.length; i++) {
                        BufferedHeader bufferedHeader = (BufferedHeader)headers[i];
                        if(!"Content-Length".equals(bufferedHeader.getName())){
                            postOperator.addHeader(headers[i]);
                        }
                    }

                    StringBody result = new StringBody("1", Charset.forName("utf-8"));
                    StringBody standard_id = new StringBody("bf15e455-c884-403c-9600-d88245928caa", Charset.forName("utf-8"));
                    StringBody check_item_id = new StringBody("f1f05f77-418f-4951-b32c-58bfaf4d42c2", Charset.forName("utf-8"));
                    StringBody harm_id = new StringBody("35f06462-63b3-43ab-b8c6-97d32a36ceca", Charset.forName("utf-8"));
                    ...
                    StringBody person_id = new StringBody("d89e2f12-f629-4a9a-a120-e90c76954255", Charset.forName("utf-8"));
                    StringBody person_name = new StringBody("蔡直文", Charset.forName("utf-8"));
                    StringBody leve = new StringBody("0", Charset.forName("utf-8"));
                    StringBody leve_name = new StringBody("检查合格", Charset.forName("utf-8"));
                    HttpEntity reqEntity = MultipartEntityBuilder.create()
                            .addPart("result", result)
                            .addPart("standard_id", standard_id)
                            .addPart("check_item_id", check_item_id)
                            .addPart("harm_id", harm_id)
