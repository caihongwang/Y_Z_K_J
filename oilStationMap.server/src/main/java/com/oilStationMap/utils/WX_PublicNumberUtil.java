...
public class WX_PublicNumberUtil {
     ...
     * @return {ApiResult}
     */
    public static Map<String, Object> dailyMessageSend(String messageJson, String appId, String secret) {
        HttpsUtil httpsUtil = new HttpsUtil();
        Map<String, String> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
        Map<String, Object> accessTokenMap = getAccessToken(appId, secret);
    ...
    public static Map<String, Object> getAccessToken() {
        ...
        Map<String, String> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
        paramMap.put("appid", OilStationMapCode.WX_PUBLIC_NUMBER_APPID);
        paramMap.put("secret", OilStationMapCode.WX_PUBLIC_NUMBER_SECRET);
     ...
     * @return ApiResult 返回信息
     */
    public static Map<String, Object> batchGetMaterial(MediaTypeUtil mediaTypeUtil, Integer offset, Integer count) {
        HttpsUtil httpsUtil = new HttpsUtil();
        Map<String, Object> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
        ...
        if (accessTokenMap != null && accessTokenMap.size() > 0) {
            ...
            resultMap = JSONObject.parseObject(resultJson, Map.class);
            totalCountStr = resultMap.get("total_count") != null ? resultMap.get("total_count").toString() : "0";
                    ...
                    if(totalCount > 0 && ((totalCount - offset) > 0)){     //确认是否查到了尽头，用来判断是否继续递归
                        ...
                        if(!"".equals(itemJsonStr)){
                            ...
                            if(itemMapList != null && itemMapList.size() > 0){
                                    ...
                                    if(contentMap != null && contentMap.size() > 0){
                                        ...
                                        if(newsItemMapList != null && newsItemMapList.size() > 0){
                                            for(Map<String, Object> newsItemMap : newsItemMapList){
                                                String url = newsItemMap.get("url")!=null?newsItemMap.get("url").toString():"";
                                                ...
                                                if(createTime_Integer > 0){
                                                    createTime = sdf.format(new Date(Long.valueOf(createTime_Integer+"000")));
                                                }
                                                ...
                                                if(updateTime_Integer > 0){
                                                    updateTime = sdf.format(new Date(Long.valueOf(updateTime_Integer+"000")));
                                                }
                                                String thumb_url = newsItemMap.get("thumb_url")!=null?newsItemMap.get("thumb_url").toString():"";
                                                ...
                                                tempMap.put("thumb_url", thumb_url);
                                ...
                                //获取的单项文章的素材小于0
                                logger.info("获取的单项文章的素材小于0");
                            }
                        } else {
                            //获取素材的json字符串为空
                            logger.info("获取素材的json字符串为空");
                        }
                        offset = offset + 20;
                    } else {
                        //停止递归
                        logger.info("没有更多的素材了");
                    }
                } else {
                    //素材总数小于0
                    logger.error("素材总数小于0");
                }
            } else {
                //获取素材总数失败
        ...
        resultMap.clear();
        resultMap.put("materialList", JSONObject.toJSONString(resultMapList));
        resultMap.put("totalCountStr", totalCountStr);
        return resultMap;
    }
     ...
     * @return ApiResult 返回信息
     */
    public static List<Map<String, Object>> batchGetAllMaterial(MediaTypeUtil mediaTypeUtil, Integer offset, Integer count) {
        HttpsUtil httpsUtil = new HttpsUtil();
        Map<String, Object> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
        List<Map<String, Object>> resultMapList = Lists.newArrayList();
    ...
    public static ResultMapDTO enterprisePayment(
        ...
        try {
            ...
            if ("SUCCESS".equals(return_code) && "SUCCESS".equals(result_code)) {
                ...
                resultMapDTO.setCode(OilStationMapCode.RED_PACKET_SEND_IS_ERROR_BUT_RESPOSE_SUCCESS.getNo());
                resultMapDTO.setMessage(resultMsg);
            }
        } catch (Exception e) {
            logger.info("企业付款失败，e : " + e.getMessage());
            resultMapDTO.setCode(OilStationMapCode.RED_PACKET_SEND_IS_ERROR.getNo());
            resultMapDTO.setMessage(OilStationMapCode.RED_PACKET_SEND_IS_ERROR.getMessage());
        }
        return resultMapDTO;
    }
...
//            IOUtils.closeQuietly(inputStream);
    ...
    public static Map<String, Object> getUserMiniProgramCode(String appId, String secret,
                                                             String page, String scene, String userMiniProgramCodePath) {
        HttpsUtil httpsUtil = new HttpsUtil();
        Map<String, String> paramMap = Maps.newHashMap();
        Map<String, Object> resultMap = Maps.newHashMap();
        Map<String, Object> accessTokenMap = getAccessToken(appId, secret);
...
//                    userMiniProgramCodePath = userMiniProgramCodePath + "uid_" +  + "miniProgramCode.jpg";
                    File dirFile = new File(userMiniProgramCodePath);
                    if (!dirFile.getParentFile().exists()) {
                        dirFile.getParentFile().mkdirs();
                    }
                    dirFile.createNewFile();
                    OutputStream out = new FileOutputStream(userMiniProgramCodePath);
                    do {
                    ...
                    out.close();
                    inStream.close();
                    userMiniProgramCodePath = userMiniProgramCodePath.substring(34);
                    String miniProgramCodeUrl = OilStationMapCode.THE_DOMAIN + userMiniProgramCodePath;
