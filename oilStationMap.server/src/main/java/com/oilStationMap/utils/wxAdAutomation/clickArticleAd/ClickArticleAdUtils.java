 ...
 */
public class ClickArticleAdUtils {

    public static final Logger logger = LoggerFactory.getLogger(ClickArticleAdUtils.class);
     ...
     * @throws Exception
     */
    public static void clickArticleAd(Map<String, Object> paramMap){
//        try{
...
//        }
        ...
        for(String nickName : nickNameList){
            ...
            paramMap.put("dicCode", nickName);        //指定 某一个点击微信文章中的广告 发送
...
//                dicCodeList.add("HuaWeiMate7ListAndShareArticleToFriendCircleLocaltion");//获取 华为 Mate 7 设备列表和配套的坐标配置
                                ...
                                if(!"".equals(startTimeStr) && !"".equals(endTimeStr)){
                                    try{
                                        ...
                                        if(currentDate.after(startTime) && currentDate.before(endTime)){
                                            ...
                                            new RealMachineDevices().clickArticleAd(shareArticleToFriendCircleParam);
                                            Thread.sleep(5000);
                                        } else {
            ...
            while (rebootDeviceNameList.size() > 0) {
                ...
                while (iterator.hasNext()) {
                    ...
                    try {
                        ...
                        new RealMachineDevices().clickArticleAd(deviceNameMap);
                        Thread.sleep(5000);
                        iterator.remove();
            ...
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如下......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如下......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如下......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如下......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如下......");
            ...
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如上......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如上......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如上......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如上......");
            logger.info("【"+nickName+"】【点击微信文章中的广告】5次次批量执行均失败的设备如上......");
            if(rebootDeviceNameList != null && rebootDeviceNameList.size() > 0){
                ...
                exceptionDevicesParamMap.put("operatorName", "点击微信文章中的广告");
