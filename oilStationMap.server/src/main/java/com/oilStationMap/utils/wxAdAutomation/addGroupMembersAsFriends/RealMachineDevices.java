...
public class RealMachineDevices implements AddGroupMembersAsFriends {
    ...
    public boolean addGroupMembersAsFriends(Map<String, Object> paramMap) throws Exception {
        ...
        try {
            ...
            logger.info("【添加群成员为好友的V群】设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】连接Appium成功....");
            Thread.sleep(10000);                                                                     //加载安卓页面10秒,保证xml树完全加载
        } catch (Exception e) {
            throw new Exception("【添加群成员为好友的V群】配置连接android驱动出现异常,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的环境是否正常运行等原因....");
        }
        for (int i = 1; i <= 30; i++) {     //每间隔5秒点击一次，持续90秒
            ...
            try {
                ...
                logger.info("【添加群成员为好友的V群】点击坐标【搜索框】成功....");
                Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
            } catch (Exception e) {
                logger.info("【添加群成员为好友的V群】点击坐标【搜索框】失败，因为微信正在建立索引....");
                if (i == 30) {
                    throw new Exception("【添加群成员为好友的V群】点击坐标【搜索框】均失败,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
                } else {
                    Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
                    logger.info("【添加群成员为好友的V群】第【" + i + "】次点击坐标【搜索框】失败，因为微信正在建立索引....");
                    continue;
                }
            }
            //3.点击坐标【搜索输入框】
            try {
                ...
                logger.info("【添加群成员为好友的V群】点击坐标【输入昵称到搜索框:text/搜索】成功....");
                Thread.sleep(1000);
                break;
            } catch (Exception e) {
                logger.info("【添加群成员为好友的V群】点击坐标【输入昵称到搜索框:text/搜索】失败....");
                try {
                    driver.findElementByAndroidUIAutomator("new UiSelector().className(\"android.widget.EditText\")").sendKeys(nickName);
                    logger.info("【添加群成员为好友的V群】点击坐标【输入昵称到搜索框:className/android.widget.EditText】成功....");
                    Thread.sleep(1000);
                    break;
                } catch (Exception e1) {
                    logger.info("【添加群成员为好友的V群】点击坐标【输入昵称到搜索框:className/android.widget.EditText】失败....");
                    if (i == 30) {
                        ...
                        Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
                        logger.info("【添加群成员为好友的V群】第【" + i + "】次点击坐标【输入昵称到搜索框:text/搜索】与【输入昵称到搜索框:className/android.widget.EditText】均失败，因为微信正在建立索引....");
                        continue;
                    }
                }
            }
        ...
        try {
            WebElement contact_WebElement = driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + groupLocaltion + "\")");
            logger.info("【添加群成员为好友的V群】检测坐标【群聊】成功....");
            Thread.sleep(2000);
        } catch (Exception e) {
            try {
                WebElement mostUsed_WebElement = driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + mostUsedLocaltion + "\")");
                logger.info("【添加群成员为好友的V群】检测坐标【最常使用】成功....");
                Thread.sleep(2000);
            } catch (Exception e1) {
                throw new Exception("【添加群成员为好友的V群】判断坐标【群聊】与【最常使用】均不存在，当前昵称【" + nickName + "】对应的可能是【微信群】或者【公众号】或者【聊天记录】....", e);
            }
        }
        ...
        try {
            ...
            logger.info("【添加群成员为好友的V群】点击坐标【昵称对应的微信好友群】通过【联系人的xpath】成功....");
        } catch (Exception e) {
            try {
                driver.findElementByXPath("//android.widget.TextView[@text=\"" + mostUsedLocaltion + "\"]/../../../android.widget.RelativeLayout[2]").click();
                logger.info("【添加群成员为好友的V群】点击坐标【昵称对应的微信好友群】通过【最常使用的xpath】成功....");
            } catch (Exception e1) {
                throw new Exception("【添加群成员为好友的V群】通过【联系人的xpath】与【最常使用的xpath】点击坐标【昵称对应的微信好友】均失败，当前昵称【\" + nickName + \"】对应的可能是【微信群】或者【公众号】或者【聊天记录】....");
            }
        }
//        try {
...
//                }
//            }
//            logger.info("【添加群成员为好友的V群】点击坐标【昵称对应的微信好友群】成功....");
//            Thread.sleep(1000);
//        } catch (Exception e) {
//            throw new Exception("【添加群成员为好友的V群】点击坐标【昵称对应的微信好友群】出现异常,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
//        }
        //6.点击坐标【右上角的三个点：聊天信息】
        try {
            driver.findElementByAndroidUIAutomator("new UiSelector().description(\"" + threePointLocaltion + "\")").click();
            logger.info("【添加群成员为好友的V群】点击坐标【右上角的三个点】成功....");
            Thread.sleep(1000);
        } catch (Exception e) {
            throw new Exception("【添加群成员为好友的V群】点击坐标【右上角的三个点】出现异常,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
        }
        //7.查看坐标【群成员总数：聊天信息(】
        ...
        try {
            ...
            logger.info("【添加群成员为好友的V群】查看【当前群的成员数量】成功....");
            Thread.sleep(1000);
        } catch (Exception e) {
            throw new Exception("【添加群成员为好友的V群】点击坐标【右上角的三个点】出现异常,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
        }
        ...
        if (groupTotalNum >= 40) {     //当群成员超过40人事，才会出现【查看全部群成员】
            ...
            try {
                ...
                logger.info("【添加群成员为好友的V群】上滑过程中寻找并点击坐标【查看全部群成员】成功...");
                Thread.sleep(1000);
            } catch (Exception e) {
                throw new Exception("【添加群成员为好友的V群】上滑过程中寻找并点击坐标【查看全部群成员】失败...");
            }
//            int i = 1;
//            while (i <= 10) {
//                try {
//                    Thread.sleep(1000);
//                    logger.info("【添加群成员为好友的V群】第【" + i + "】次，scroll下滑中寻找坐标【查看全部群成员】...");
...
//                } catch (Exception e) {
//                    logger.info("【添加群成员为好友的V群】scroll下滑中寻找坐标【查看全部群成员】...");
//                }
//                try {
//                    Thread.sleep(1000);
//                    driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + checkAllGroupMembers + "\")").click();
//                    Thread.sleep(1000);
//                    logger.info("【添加群成员为好友的V群】点击坐标【查看全部群成员】成功....");
//                    break;
//                } catch (Exception e) {
//                    logger.info("【添加群成员为好友的V群】当前为群成员界面，正在往上滑动，寻找坐标【查看全部群成员】");
//                }
//                i++;
//            }
            //8.下滑，回到群成员的顶部
            try {
                ...
                logger.info("【添加群成员为好友的V群】【下滑】已滑到群成员的顶部成功....");
            } catch (Exception e) {
                logger.info("【添加群成员为好友的V群】【下滑】已滑到群成员的顶部异常....");
            }
            ...
            if (groupMembersMap.size() != groupTotalNum) {
                    ...
                    if (linearWebElementList != null && linearWebElementList.size() > 0) {
                        ...
                        if (cyclesNumber >= 30) {           //当循环下拉的次数超过30次时，则强制终止循环，主要是群成员中存在昵称相同的人，导致groupMembersMap无法添加进去，导致数量达不到真实的群成员数量
                            ...
                            if (groupMembersMap.size() >= groupTotalNum) {        //当群成员数量全部统计完成时就跳出循环
                                ...
                                try {
                                    logger.info("【添加群成员为好友的V群】第【" + cyclesNumber + "】次，scroll上滑中,查看更过群成员，直到groupMembersMap的大小达到" + groupTotalNum + "，当前groupMembersMap的大小为：" + groupMembersMap.size() + "...");
                                    driver.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true)).scrollForward()");
                                } catch (Exception e) {
                                    logger.info("【添加群成员为好友的V群】scroll上滑中,查看更过群成员....");
                                }
                                cyclesNumber++;
                            }
                        }
                ...
                logger.info("【添加群成员为好友的V群】点击坐标【上滑同时检测坐标查看全部群成员】成功....");
                Thread.sleep(1000);
            }
            //10.循环点击群成员
...
//                }
                if ("false".equals(isAddFlag)) {
                    try {
                        ...
                        try {
                            ...
                            logger.info("【添加群成员为好友的V群】点击坐标【搜索:text】寻找群成员【" + groupMemberNickName + "】成功....");
                        } catch (Exception e) {
                            try {
                                driver.findElementByAndroidUIAutomator("new UiSelector().className(\"android.widget.EditText\")").sendKeys(groupMemberNickName);
                                logger.info("【添加群成员为好友的V群】点击坐标【搜索:className/android.widget.EditText】寻找群成员【" + groupMemberNickName + "】成功....");
                            } catch (Exception e1) {
                                throw new Exception("【添加群成员为好友的V群】点击坐标【搜索:text】与【搜索:className/android.widget.EditText】寻找群成员【" + groupMemberNickName + "】均失败....");
                            }
                        }
                        Thread.sleep(1500);
                        ...
                        try {
                            ...
                            if (gridWebElement != null) {
                                logger.info("【添加群成员为好友的V群】检测坐标【android.widget.GridView】成功，获取昵称对应的群成员列表信息....");
                                Thread.sleep(1000);
                                try {
                                    ...
                                    if (linearWebElementList != null) {
                                        logger.info("【添加群成员为好友的V群】检测坐标【android.widget.TextView】成功，获取昵称对应的群成员列表信息，共【" + linearWebElementList.size() + "】个....");
                                        Thread.sleep(1000);
                                    } else {
                                        logger.info("【添加群成员为好友的V群】检测坐标【android.widget.TextView】成功，获取昵称对应的群成员列表信息，共【" + linearWebElementList.size() + "】个....");
                                    }
                                } catch (Exception e) {
                                    logger.info("【添加群成员为好友的V群】检测坐标【android.widget.TextView】失败，获取昵称对应的群成员列表信息....");
                                }
                            }
                        } catch (Exception e) {
                            logger.info("【添加群成员为好友的V群】检测坐标【android.widget.GridView】失败，获取昵称对应的群成员列表信息....");
                        }
                        if (linearWebElementList != null && linearWebElementList.size() > 0) {
                            for (WebElement webElement : linearWebElementList) {            //准备点击群成员头像，开始点击
                                try {
                                    ...
                                    if (!groupMemberNickName_Temp.equals(groupMemberNickName)) {      //防止一个短昵称对应多个群成员
                                        logger.info("【添加群成员为好友的V群】未发现昵称对应的群成员【" + groupMemberNickName + "】，当前昵称【" + groupMemberNickName_Temp + "】....");
                                        continue;
                                    } else {
                                        logger.info("【添加群成员为好友的V群】找到昵称对应的群成员【" + groupMemberNickName + "】准备添加为好友....");
                                    }
                                    webElement.click();         //当一个昵称对应太多群成员时，在遍历过程中可能会应为时间太长而无法定位致使点击异常
                                    logger.info("【添加群成员为好友的V群】点击群成员【" + groupMemberNickName + "】成功....");
                                    Thread.sleep(4000);
                                    //12.检测坐标【发消息】
                                    try {
                                        ...
                                        if (sendMessageBtn_Element != null) {
                                            logger.info("【添加群成员为好友的V群】检测坐标【发消息】成功，您与【" + groupMemberNickName + "】已是好友，则直接下一个群成员坐标....");
                                            Thread.sleep(1000);
                                            driver.pressKeyCode(AndroidKeyCode.BACK);                   //返回【聊天成员界面】
                                            logger.info("【添加群成员为好友的V群】检测坐标【发消息】之后并返回【聊天成员界面】成功....");
                                            Thread.sleep(4000);
                                            groupMember.put("isAddFlag", "true");
                                            continue;
                                        }
                                    } catch (Exception e) {
                                        logger.info("【添加群成员为好友的V群】检测坐标【发消息】异常，可能是当前群成员不是您的好友，从而没有找到【发消息】按钮....");
                                    }
                                    //13.点击坐标【添加到通讯录】
                                    try {
                                        driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + aadToContactBookLocaltion + "\")").click();
                                        logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】成功....");
                                        Thread.sleep(6000);
                                    } catch (Exception e) {
                                        logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】异常，可能是没有找到【添加到通讯录】按钮....");
                                    }
                                    ...
                                    try {
                                        WebElement sendMessageBtn_Element = driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + sendMessageBtnLocaltion + "\")");
                                        if (sendMessageBtn_Element != null) {
                                            logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，检测坐标【发消息】成功，在点击坐标【添加到通讯录】直接被对方【" + groupMemberNickName + "】通过为好友（即通知发送成功），则直接下一个群成员坐标....");
                                            Thread.sleep(1000);
                                            driver.pressKeyCode(AndroidKeyCode.BACK);                   //返回【聊天成员界面】
                                            logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，检测坐标【发消息】之后并返回【聊天成员界面】成功....");
                                            Thread.sleep(4000);
                                            groupMember.put("isAddFlag", "true");
                                            addFriendNum++;
                                            continue;
                                        ...
                                        logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，检测坐标【发消息】异常，当前用户没有在点击坐标【添加到通讯录】直接添加为好友....");
                                    }
                                    ...
                                    try {
                                        ...
                                        if (privacyContent_Element != null) {
                                            logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，显示内容【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】成功，直接下一个群成员坐标....");
                                            Thread.sleep(1000);
                                            driver.pressKeyCode(AndroidKeyCode.BACK);                   //退出【弹窗】  TODO：后续可采用点击坐标【确定】进行退出【弹窗】
                                            logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，显示内容【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】成功之后退出【弹窗】成功....");
                                            Thread.sleep(4000);
                                            driver.pressKeyCode(AndroidKeyCode.BACK);                   //返回【聊天成员界面】
                                            logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，显示内容【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】成功之后返回【聊天成员界面】成功....");
                                            Thread.sleep(4000);
                                            groupMember.put("isAddFlag", "true");
                                            continue;
                                        }
                                        ...
                                        try {
                                            ...
                                            if (privacyContent_Element != null) {
                                                logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，显示内容【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】的【确定】成功，直接下一个群成员坐标....");
                                                Thread.sleep(1000);
                                                driver.pressKeyCode(AndroidKeyCode.BACK);                   //退出【弹窗】
                                                logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，显示内容【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】的【确定】成功之后退出【弹窗】成功....");
                                                Thread.sleep(4000);
                                                driver.pressKeyCode(AndroidKeyCode.BACK);                   //返回【聊天成员界面】
                                                logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，显示内容【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】的【确定】成功之后返回【聊天成员界面】成功....");
                                                Thread.sleep(4000);
                                                groupMember.put("isAddFlag", "true");
                                                continue;
                                            }
                                        } catch (Exception e1) {
                                            logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，【未】显示内容【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】，可能是没有找到【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】....");
                                        }
                                    }
                                    ...
                                    try {
                                        ...
                                        logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，检测坐标【发送添加朋友申请】成功....");
                                        Thread.sleep(1000);
                                    } catch (Exception e) {
                                        logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，检测坐标【发送添加朋友申请】异常，以为【单个群成员简介】显示push【对方账号异常，无法添加朋友。】，消失得很快，无法捕捉，则直接下一个群成员坐标....");
                                        Thread.sleep(1000);
                                        driver.pressKeyCode(AndroidKeyCode.BACK);                   //返回【聊天成员界面】
                                        logger.info("【添加群成员为好友的V群】点击坐标【添加到通讯录】后，检测坐标【发送添加朋友申请】之后并返回【聊天成员界面】成功....");
                                        Thread.sleep(4000);
                                        groupMember.put("isAddFlag", "true");
                                        addFriendNum++;
                                        continue;
                                    ...
                                    try {
                                        ...
                                        logger.info("【添加群成员为好友的V群】点击坐标【发送添加朋友申请-输入框】【xpath】并填写内容成功....");
                                        Thread.sleep(1000);
                                    } catch (Exception e) {
                                        logger.info("【添加群成员为好友的V群】点击坐标并输入【发送添加朋友申请-输入框】【xpath】异常，直接进入下一步....");
                                    }
                                    //14.2.点击坐标【朋友圈】，主要是为了选择权限
                                    try {
                                        driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + friendCircleLocaltion + "\")").click();
                                        logger.info("【添加群成员为好友的V群】点击坐标【朋友圈】成功....");
                                        Thread.sleep(1000);
                                    } catch (Exception e) {
                                        logger.info("【添加群成员为好友的V群】点击坐标【朋友圈】异常，可能是当前微信版本锅底，没有找到【朋友圈】按钮....");
                                    }
                                    //14.3.点击坐标【发送】
                                    try {
                                        driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + sendBtnLocaltion + "\")").click();
                                        logger.info("【添加群成员为好友的V群】点击坐标【发送】成功....");
                                        Thread.sleep(5000);
                                    } catch (Exception e) {
                                        logger.info("【添加群成员为好友的V群】点击坐标【发送】异常，可能出现了【对方帐号异常，无法添加朋友。】或者【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】....");
                                    }
                                    ...
                                    try {
                                        ...
                                        groupMember.put("isAddFlag", "true");
                                        addFriendNum++;
                                        logger.info("【添加群成员为好友的V群】点击坐标【发送】后，检测坐标【添加到通讯录】成功，当前请求好友【" + groupMemberNickName + "】通知发送成功....");
                                        Thread.sleep(1000);
                                    } catch (Exception e) {
                                        logger.info("【添加群成员为好友的V群】点击坐标【发送】后，检测坐标【添加到通讯录】异常，可能是当前用户【" + groupMemberNickName + "】在发送页面才显示【对方账号异常，无法添加朋友。】....");
                                        groupMember.put("isAddFlag", "true");
                                        driver.pressKeyCode(AndroidKeyCode.BACK);                   //返回【单个群成员简介】
                                        logger.info("【添加群成员为好友的V群】点击坐标【发送】后，检测坐标【添加到通讯录】异常之后返回【单个群成员简介】成功....");
                                        Thread.sleep(4000);
                                    }
                                    Thread.sleep(1000);
                                    driver.pressKeyCode(AndroidKeyCode.BACK);                   //返回【聊天成员界面】
                                    logger.info("【添加群成员为好友的V群】返回【聊天成员界面】成功....");
                                    Thread.sleep(4000);
                                } catch (Exception e) {
                                    logger.info("【添加群成员为好友的V群】点击群成员【" + groupMemberNickName + "】准备添加为好友异常，有可能一个昵称对应多个群成员，在循环遍历过程中WebElement超时了，无法定位点击....，e ：", e);
                                }
                                break;
                            }
                            ...
                            if (!"true".equals(groupMember.get("isAddFlag"))) {
                                ...
                                logger.info("【添加群成员为好友的V群】未发现群成员用户【" + groupMemberNickName + "】，当前昵称在此群中对应的群成员太多无法快速定位，直接下一个....");
                            }
                        } else {
                            groupMember.put("isAddFlag", "true");           //根据群昵称找不到群成员的，则默认为已添加过
                            logger.info("【添加群成员为好友的V群】未发现群成员用户【" + groupMemberNickName + "】，直接下一个....");
                        }
                    } catch (Exception e) {
                        logger.info("【添加群成员为好友的V群】在搜索框输入【" + groupMemberNickName + "】查找群成员异常....，e ：", e);
                    } finally {
                        ...
                        while (true) {
                            try {
                                ...
                                try {
                                    ...
                                    logger.info("【添加群成员为好友的V群】点击坐标【搜索:text/" + groupMemberNickName + "】清空【群成员搜索框】成功....");
                                    break;
                                } catch (Exception e) {
                                    logger.info("【添加群成员为好友的V群】点击坐标【搜索:text/" + groupMemberNickName + "】清空【群成员搜索框】失败....");
                                    try {
                                        driver.findElementByAndroidUIAutomator("new UiSelector().className(\"android.widget.EditText\")").clear();
                                        logger.info("【添加群成员为好友的V群】点击坐标【搜索:className/android.widget.EditText】清空【群成员搜索框】成功....");
                                        break;
                                    } catch (Exception e1) {
                                        throw new Exception("【添加群成员为好友的V群】点击坐标【搜索:text/" + groupMemberNickName + "】与【搜索:className/android.widget.EditText】清空【群成员搜索框】均失败....");
                                    }
                                }
                            } catch (Exception e) {
            ...
            logger.info("【添加群成员为好友的V群】群成员未超过40人，则不添加当前群的成员为好友....");
        }
