...
//        }
        ...
        for (String nickName : nickNameList) {
            ...
            paramMap.put("dicCode", nickName);        //指定 某一个微信昵称 群
...
//                dicCodeList.add("HuaWeiMate7ListAndChatByNickNameLocaltion");//获取 华为 Mate 7 设备列表和配套的坐标配置
                        ...
                        if (deviceNameList != null && deviceNameList.size() > 0) {
                            for (Map<String, Object> deviceNameMap : deviceNameList) {
                                ...
                                try {
                                    ...
                                    new RealMachineDevices().addGroupMembersAsFriends(chatByNickNameParam, sw);
                                    Thread.sleep(5000);
                ...
                logger.info("根据微信群昵称添加群成员为好友工具 失败.");
            }
            if(rebootDeviceNameList.size() > 0){
                sw.split();
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如下，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如下，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如下，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如下，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如下，总共花费 " + sw.toSplitString() + " 秒....");
                ...
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如上，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如上，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如上，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如上，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】批量执行均失败的设备如上，总共花费 " + sw.toSplitString() + " 秒....");
...
//                    }
                ...
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】全部执行成功，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】全部执行成功，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】全部执行成功，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】全部执行成功，总共花费 " + sw.toSplitString() + " 秒....");
                logger.info("【"+nickName+"】【根据微信群昵称添加群成员为好友工具】全部执行成功，总共花费 " + sw.toSplitString() + " 秒....");
