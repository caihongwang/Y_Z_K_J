            ...
            if (list != null && list.size() > 0) {
                ...
                if (deviceNameList != null && deviceNameList.size() > 0) {
                    for (Map<String, Object> deviceNameMap : deviceNameList) {
                        ...
                        //获取设备编码
                        deviceName =
                                agreeToJoinTheGroupParam.get("deviceName") != null ?
                                        agreeToJoinTheGroupParam.get("deviceName").toString() :
                                        null;
                        //当前设备描述
                        deviceNameDesc =
                                agreeToJoinTheGroupParam.get("deviceNameDesc") != null ?
                                        agreeToJoinTheGroupParam.get("deviceNameDesc").toString() :
                                        null;
                        ...
                        try {
                            if (isExecuteFlag) {
                                //开始【同意进群】
                                logger.info("【同意进群】设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】即将开始发送....");
...
//                    Thread.sleep(1000);
                    ...
                    try {
                        if (index % 4 == 0) {
                            ...
                            logger.info("【同意进群】设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】重启成功...");
                        }
                    } catch (Exception e1) {
                        logger.info("【同意进群】设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】重启失败...");
                    }
                }
                index++;
            }
            if (reboot_agreeToJoinTheGroupParam.size() > 0) {
                logger.info("【同意进群】设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】15次重新执行均失败....");
                logger.info("【同意进群】设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】15次重新执行均失败....");
                logger.info("【同意进群】设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】15次重新执行均失败....");
                logger.info("【同意进群】设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】15次重新执行均失败....");
                logger.info("【同意进群】设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】15次重新执行均失败....");
