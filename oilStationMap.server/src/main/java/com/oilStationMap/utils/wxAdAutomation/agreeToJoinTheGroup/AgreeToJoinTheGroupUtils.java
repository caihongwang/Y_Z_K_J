...
public class AgreeToJoinTheGroupUtils {
    ...
    public static void agreeToJoinTheGroup(Map<String, Object> paramMap) throws Exception {
        ...
        for (String currentDateStr : currentDateList) {
            ...
            dicCodeList.add("HuaWeiListAndAgreeToJoinTheGroupLocaltion");//获取 华为 Mate 8 设备列表和配套的坐标配置
                    ...
                    if (deviceNameList != null && deviceNameList.size() > 0) {
                        for (Map<String, Object> deviceNameMap : deviceNameList) {
                            ...
                            try {
                                ...
                                String startHour =
                                        agreeToJoinTheGroupParam.get("startHour") != null ?
                                                agreeToJoinTheGroupParam.get("startHour").toString() :
                                                "";
                                String currentHour = new SimpleDateFormat("HH").format(currentDate);
                                if (startHour.equals(currentHour)) {
                                    deviceName = agreeToJoinTheGroupParam.get("deviceName").toString();
                                    deviceNameDesc = agreeToJoinTheGroupParam.get("deviceNameDesc").toString();
                                    //开始同意进群
                                    logger.info("设备编码【" + agreeToJoinTheGroupParam.get("deviceName") + "】设备描述【" + agreeToJoinTheGroupParam.get("deviceNameDesc") + "】将同意进群即将开始发送....");
                                    new RealMachineDevices().agreeToJoinTheGroup(agreeToJoinTheGroupParam);
                                    Thread.sleep(5000);
//                                    //重新操作
//                                    reboot_agreeToJoinTheGroupParam.putAll(agreeToJoinTheGroupParam);
                                } else {
                                    //下一个设备
                                    logger.info("设备编码【" + agreeToJoinTheGroupParam.get("deviceName") + "】设备描述【" + agreeToJoinTheGroupParam.get("deviceNameDesc") + "】，当前设备的执行时间第【" + startHour + "】小时，当前时间是第【" + currentHour + "】小时....");
                                    continue;
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
            ...
            while (reboot_agreeToJoinTheGroupParam.size() > 0) {
                ...
                try {
                    logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】将同意进群即将开始发送....");
                    new RealMachineDevices().agreeToJoinTheGroup(reboot_agreeToJoinTheGroupParam);
            ...
            if (reboot_agreeToJoinTheGroupParam.size() > 0) {
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如下....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如下....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如下....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如下....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如下....");
                ...
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如上....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如上....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如上....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如上....");
                logger.info("设备编码【" + deviceName + "】设备描述【" + deviceNameDesc + "】【同意进群】5次次批量执行均失败的设备如上....");
                ...
                exceptionDevicesParamMap.put("operatorName", "同意进群");
