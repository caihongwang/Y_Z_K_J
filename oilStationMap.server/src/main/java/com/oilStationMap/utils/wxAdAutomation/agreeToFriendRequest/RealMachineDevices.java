...
//            driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS);
                ...
                if (linearWebElementList != null && linearWebElementList.size() > 0) {
                    for (WebElement webElement : linearWebElementList) {
                        ...
                        try {
                            ...
                            logger.info("【同意好友请求】当前不是聊天好友元素....");
                        }
                    }
                    if (cyclesNumber >= maxCyclesNumber) {           //当循环下拉的次数超过30次时，则强制终止循环，主要是群成员中存在昵称相同的人，导致groupMembersMap无法添加进去，导致数量达不到真实的群成员数量
                        //停止循环
                        logger.info("【同意好友请求】已滑到聊天界面的底部，第【" + cyclesNumber + "】次，scroll上滑中,检测当前页面聊天好友信息，当前 chatFriendsSet 的大小为：" + chatFriendsSet.size() + "...");
                        break;
                    } else {
                        try {
                            logger.info("【同意好友请求】第【" + cyclesNumber + "】次，scroll上滑中,检测当前页面聊天好友信息，当前 chatFriendsSet 的大小为：" + chatFriendsSet.size() + "...");
                            driver.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector().scrollable(true)).scrollForward()");
                        } catch (Exception e) {
                            logger.info("【同意好友请求】scroll上滑中,检测当前页面聊天好友信息....");
                        }
                        cyclesNumber++;
                    }
                }
        ...
        for (String chatFriendNickName : chatFriendsSet) {
            System.out.println("【同意好友请求】第【" + chatFriendsIndex + "】个好友 " + " ---->>> " + chatFriendNickName);
            chatFriendsIndex++;
        }
        System.out.println("=============================================================================================");
        System.out.println("=============================================================================================");
...
//            }
            ...
            if (chatFriendNickName.endsWith("…")) {
                logger.info("【同意好友请求】检测昵称末尾包含\"…\"，处理之前昵称【" + chatFriendNickName + "】....");
                chatFriendNickName = chatFriendNickName.substring(0, chatFriendNickName.length() - 1);
                logger.info("【同意好友请求】检测昵称末尾包含\"…\"，处理之后昵称【" + chatFriendNickName + "】....");
            }
            ...
            for (int i = 1; i <= 30; i++) {     //每间隔5秒点击一次，持续90秒
                ...
                try {
                    ...
                    logger.info("【同意好友请求】点击坐标【搜索框】成功....");
                    Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
                } catch (Exception e) {
                    logger.info("【同意好友请求】点击坐标【搜索框】失败，因为微信正在建立索引....");
                    if (i == 30) {
                        throw new Exception("【同意好友请求】点击坐标【搜索框】均失败,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
                    } else {
                        Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
                        logger.info("【同意好友请求】第【" + i + "】次点击坐标【搜索框】失败，因为微信正在建立索引....");
                        continue;
                    }
                }
                //3.点击坐标【搜索输入框】
                try {
                    ...
                    logger.info("【同意好友请求】点击坐标【输入昵称到搜索框:text/搜索】成功....");
                    Thread.sleep(1000);
                    break;
                } catch (Exception e) {
                    logger.info("【同意好友请求】点击坐标【输入昵称到搜索框:text/搜索】失败....");
                    try {
                        driver.findElementByAndroidUIAutomator("new UiSelector().className(\"android.widget.EditText\")").sendKeys(chatFriendNickName);
                        logger.info("【同意好友请求】点击坐标【输入昵称到搜索框:className/android.widget.EditText】成功....");
                        Thread.sleep(1000);
                        break;
                    } catch (Exception e1) {
                        logger.info("【同意好友请求】点击坐标【输入昵称到搜索框:className/android.widget.EditText】失败....");
                        if (i == 30) {
                            throw new Exception("【同意好友请求】点击坐标【输入昵称到搜索框:text/搜索】与【输入昵称到搜索框:className/android.widget.EditText】均失败,请检查设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】的应用是否更新导致坐标变化等原因....");
                        } else {
                            Thread.sleep(5000);         //此处会创建索引，会比较费时间才能打开
                            logger.info("【同意好友请求】第【" + i + "】次点击坐标【输入昵称到搜索框:text/搜索】与【输入昵称到搜索框:className/android.widget.EditText】均失败，因为微信正在建立索引....");
                            continue;
                        }
                    }
                }
            ...
            try {
                ...
                try {
                    ...
                    logger.info("【同意好友请求】判断坐标【联系人】与【最常使用】均不存在，当前昵称【" + chatFriendNickName + "】对应的可能是【微信群】或者【公众号】或者【聊天记录】....");
                }
            }
...
//            }
            ...
            try {
                ...
                logger.info("【同意好友请求】点击坐标【添加到通讯录】成功....");
                Thread.sleep(6000);
            } catch (Exception e) {
                logger.info("【同意好友请求】点击坐标【添加到通讯录】异常，可能是没有找到【添加到通讯录】按钮....");
            }
            ...
            try {
                ...
                if (privacyContent_Element != null) {
                    logger.info("【同意好友请求】点击坐标【添加到通讯录】后，检测坐标【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】成功，直接下一个群成员坐标....");
                    isYourFriendFlag = true;
                    theAgreeNum++;
                }
                Thread.sleep(2000);
            ...
            try {
                ...
                logger.info("【同意好友请求】点击坐标【朋友圈】成功....");
                Thread.sleep(6000);
            } catch (Exception e) {
                logger.info("【同意好友请求】点击坐标【朋友圈】异常，可能是当前微信版本锅底，没有找到【朋友圈】按钮....");
            }
            //点击坐标【发送】
            try {
                ...
                logger.info("【同意好友请求】点击坐标【发送】成功....");
                Thread.sleep(5000);
                theAgreeNum++;
            } catch (Exception e) {
                logger.info("【同意好友请求】点击坐标【发送】异常，可能出现了【对方帐号异常，无法添加朋友。】或者【由于对方的隐私设置，你无法通过群聊将其添加至通讯录】....");
            }
            //检测坐标【添加到通讯录】
            try {
                WebElement aadToContactBook_Element = driver.findElementByAndroidUIAutomator("new UiSelector().text(\"" + aadToContactBookLocaltion + "\")");
                logger.info("【同意好友请求】点击坐标【发送】后，检测坐标【添加到通讯录】成功，同意好友请求【" + chatFriendNickName + "】通知发送成功....");
            } catch (Exception e) {
...
//            Thread.sleep(2000);
        ...
        try {
            ...
            logger.info("【同意好友请求】点击坐标【清空】异常....");
        }
        logger.info("【同意好友请求】设备描述【" + deviceNameDesc + "】设备编码【" + deviceName + "】操作【" + action + "】 同意好友请求【" + theAgreeNum + "】个发送成功....");
        return true;
    }
