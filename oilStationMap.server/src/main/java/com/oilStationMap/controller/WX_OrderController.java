...
import com.oilStationMap.handler.WX_OrderHandler;
import com.oilStationMap.utils.HttpUtil;
import com.oilStationMap.utils.PayUtil;
...
public class WX_OrderController {
    ...
    @Autowired
    private WX_OrderHandler wxOrderHandler;
    ...
    public Map<String, Object> requestWxPayUnifiedOrder(HttpServletRequest request) {
        ...
        try {
            ResultMapDTO resultMapDTO = wxOrderHandler.requestWxPayUnifiedOrder(paramMap);
    ...
    public void wxPayUnifiedOrderNotify(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if ("SUCCESS".equals(returnCode)) {
            //验证签名是否正确
            if (PayUtil.verify(PayUtil.createLinkString(map), (String) map.get("sign"), OilStationMapCode.WX_PAY_API_SECRET, "utf-8")) {
                /**此处添加自己的业务逻辑代码start**/
