 ...
 */
@Component
public class WX_OilStationOperatorHandler {

    private static final Logger logger = LoggerFactory.getLogger(WX_OilStationOperatorHandler.class);

    @Autowired
    private WX_OilStationOperatorService wxOilStationOperatorService;

    public ResultMapDTO cashOilStationOperatorRedPacket(Map<String, String> paramMap) {
        logger.info("在hanlder中领取或者提现加油站操作红包-cashOilStationOperatorRedPacket,请求-paramMap:" + paramMap);
        ResultMapDTO resultMapDTO = new ResultMapDTO();
        Map<String, Object> objectParamMap = MapUtil.getObjectMap(paramMap);
        if (paramMap.size() > 0) {
            try {
                resultMapDTO = wxOilStationOperatorService.cashOilStationOperatorRedPacket(objectParamMap);
            } catch (Exception e) {
