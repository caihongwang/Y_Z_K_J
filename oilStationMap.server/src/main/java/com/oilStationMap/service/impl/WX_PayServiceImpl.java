...
import com.oilStationMap.utils.WX_PayUtil;
...
public class WX_PayServiceImpl implements WX_PayService {
    ...
    public ResultMapDTO unifiedOrderPay(Map<String, Object> paramMap) {
        ...
        if (!"".equals(openId) && !"".equals(spbillCreateIp)) {
            if ("".equals(payMoney)) {
                ...
                // 通知地址
                String notifyUrl = spbillCreateIp + OilStationMapCode.WX_PAY_NOTIFY_URL;
                // 交易类型
                String tradeType = OilStationMapCode.WX_PAY_TRADE_TYPE;
                // 微信支付商户的api秘钥
                String apiSecret = OilStationMapCode.WX_PAY_API_SECRET;
    ...
    public ResultMapDTO toOauthUrlForPaymentPage(Map<String, Object> paramMap) {
        Map<String, Object> resultMap = Maps.newHashMap();
        ResultMapDTO resultMapDTO = new ResultMapDTO();
        String calbackUrl = OilStationMapCode.WX_PAY_DOMAIN + "/wxPay/getPaymentPage";
        //准备参数
        try{
